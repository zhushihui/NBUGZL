define(function(require, exports, module) {
	...
	var jsgrgzlhzSave = require('./jsgrgzlhzSave');
	var jsgrgzlhzView = require('./jsgrgzlhzView');

	var viewConfig = {
		initialize: function() {
			var view = utils.loadCompiledPage('jsgrgzlhz');
            this.$rootElement.html(view.render({}), true);
            this.pushSubView([jsgrgzlhzSave]);
            this.initView();
        ...
        actionAdd: function(){
        	var jsgrgzlhzNewTpl = utils.loadCompiledPage('jsgrgzlhzSave');
        	$.bhPaperPileDialog.show({
        		content: jsgrgzlhzNewTpl.render({}),
        		title: "新建",
        		ready: function($header, $body, $footer){
        			jsgrgzlhzSave.initialize();
            	}
            });
        },
 	   ...
 	   actionEdit: function(e){
        	...
        	var jsgrgzlhzEditTpl = utils.loadCompiledPage('jsgrgzlhzSave');
        	var data = WIS_EMAP_SERV.getData(bs.api.pageModel, 'jsgrgzlhz', {WID:id});
        	
        	$.bhPaperPileDialog.show({
        		content: jsgrgzlhzEditTpl.render({}),
        		title: "编辑",
        		ready: function($header, $body, $footer){
        			jsgrgzlhzSave.initialize();
        ...
        actionDetail: function(e){
        	var id = $(e.target).attr("data-x-wid");
        	var jsgrgzlhzViewTpl = utils.loadCompiledPage('jsgrgzlhzSave');
        	var data = WIS_EMAP_SERV.getData(bs.api.pageModel, 'jsgrgzlhz', {WID:id});
        	
        	$.bhPaperPileDialog.show({
        		content: jsgrgzlhzViewTpl.render({}),
        		title: "查看",
        		ready: function($header, $body, $footer){
        			jsgrgzlhzView.initialize(data.rows[0]);
            	}
            });
        },
...
//        	});
		...
		_initAdvanceQuery: function() {
            var searchData = WIS_EMAP_SERV.getModel(bs.api.pageModel, 'jsgrgzlhz', "search");
